...
const VOLUME_GATE = 4;
const THRESHOLDS = [2, 2.5, 3, 3.5, 4, 5];

type Rule = { label: string; fires: (row: ObservedRow) => boolean };

/**
 * The two rules the market-wide-day decision is between.
 *
 ...
 * into a measurement.
 */
function rules(threshold: number): Rule[] {
  return [
    {
      label: `flat OR      >= ${threshold}`,
      fires: (row) =>
        mag(row.volatilityAdjusted) >= threshold ||
        mag(row.relativeAdjusted) >= threshold ||
    ...
    },
    {
      ...
      fires: (row) =>
        row.kind === "benchmark"
          ? mag(row.volatilityAdjusted) >= threshold
          : mag(row.relativeAdjusted) >= threshold || (row.volume ?? 0) >= VOLUME_GATE,
    },
  ];
}
...
for (const threshold of THRESHOLDS) {
  for (const rule of rules(threshold)) {
